...
3. **Launch the Server**:
   ...
   ```
   _The interactive API documentation will be available at: http://127.0.0.1:8000/docs_

### Release Build Configuration

For release builds, point the Flutter app at a deployed backend and pass the user/session defaults at build time:

```bash
flutter build apk --release \
  --dart-define=VISTA_API_BASE_URL=https://api.example.com/api \
  --dart-define=VISTA_USER_ID=your-user-id
```

If you deploy the FastAPI backend behind a web frontend or reverse proxy, set:

```bash
export CORS_ALLOW_ORIGINS=https://app.example.com,https://staging.example.com
```

The app also accepts optional `VISTA_AUTH_TOKEN`, `VISTA_DEFAULT_LATITUDE`, and `VISTA_DEFAULT_LONGITUDE` build-time overrides.
...
- If a WebSocket connection drops during a walk, immediately shift the UI to **Fallback Simulation Mode**, showing local GPS status and playing a chime: _"Connection lost. Operating on local cane sensor mode."_
